...
def clone_template(template: str, slug: str, build_dir: str = "") -> dict:
    ...
    Build dir defaults to the durable ``<project>/.cache/mockups/<slug>/``
    (see ``app/utils/mockup_build.py``) so the clone survives container
    restarts. ``node_modules`` + ``dist`` + ``.cache`` are still excluded
    from the copy — they'll be regenerated by ``pnpm install`` + ``build``.
    """
...
def build(slug: str, build_dir: str = "", client_ts: str = "", brand_ts: str = "", asset_paths: list[str | None] | None = None, timeout_s: int = 180) -> dict:
    """Run pnpm install + pnpm build in the cloned template.

    If ``client_ts``, ``brand_ts``, and ``asset_paths`` are provided and the
    cached input hash matches, the build is skipped entirely and the
    existing ``dist/`` is reused. This is the Session 19 optimisation —
    avoids re-running pnpm install + pnpm build (typically 40-90s) when
    the mockup is being rebuilt for a lead whose inputs haven't changed.
    """
    if not build_dir:
        build_dir = str(mockup_project_dir(slug))
    project = project_dir_for(slug, build_dir)
    ...
    # Skip-rebuild check: if hash matches last successful build, reuse dist/.
    # Caller must pass client_ts + brand_ts + asset_paths to enable.
    if client_ts and brand_ts is not None:
        from app.utils.mockup_build import should_skip_build, write_build_hash, compute_input_hash
        if should_skip_build(slug, client_ts, brand_ts, asset_paths or []):
            dist = project / "dist"
            log.info("mockup_build_skipped", slug=slug, reason="input_hash_unchanged")
            return {
                "install_ms": 0,
                "build_ms": 0,
                "total_ms": 0,
    ...
    # Record the input hash so the next build can skip-rebuild if inputs unchanged.
    if client_ts and brand_ts is not None:
        from app.utils.mockup_build import write_build_hash, compute_input_hash
        h = compute_input_hash(client_ts, brand_ts, asset_paths or [])
        write_build_hash(slug, h)
    return {
